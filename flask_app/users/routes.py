...
def account():
    ...
    b_64 = get_b64_img(current_user.username)
    if request.method == "POST":
        ...
        if update_profile_pic_form.submit_picture.data and update_profile_pic_form.validate():
            ...
            current_user.save()
            return redirect(url_for("users.account", image=b_64))
        
    return render_template("account.html", update_username_form=update_username_form, update_profile_pic_form = update_profile_pic_form, image = b_64)